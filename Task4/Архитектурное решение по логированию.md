...
```mermaid
flowchart LR
  Shop[Shop API] --> FB[Log Agent (Fluent Bit/Vector)]
  CRM[CRM API] --> FB
  MES[MES API] --> FB
  RMQ[(RabbitMQ)] --> FB

  FB --> LogStore[(OpenSearch / Loki)]
  UI[Dashboards (OpenSearch Dashboards / Grafana)] --> LogStore
  Alert[Alerting rules] --> Ops[Support/On-call]

  classDef new fill:#fff2cc,stroke:#cc9900,stroke-width:2px;
  class FB,LogStore,UI,Alert new;
```
